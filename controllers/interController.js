...
const inter = {
    readAll: async (req, res) => {
        const inters = await Inter.findAll();
        try {
            if (!inters.length)
    ...
    read: async (req, res) => {
        ...
        const inter = await Inter.find(interId);
        try {
            ...
            res.send(`find successfully: ${inter}`);
        } catch (err) {
    ...
    write: async (req, res) => {
        try {
            const result = await Inter.create(req.body)
            res.send(result);
        } catch (err) {
            res.status(500).send(err)
        }
    ...
    delete: async (req, res) => {
        try {
            const interId = req.params.interId;
            const result = await Inter.delete(interId)
            res.sendStatus(200)
        } catch (err) {
            res.status(500).send(err)
        }
