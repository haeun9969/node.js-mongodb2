...
const society = {
    readAll: async (req, res) => {
        const societys = await Society.findAll();
        try {
            if (!societys.length)
    ...
    read: async (req, res) => {
        ...
        const society = await Society.find(societyId);
        try {
            ...
            res.send(`find successfully: ${society}`);
        } catch (err) {
    ...
    write: async (req, res) => {
        try {
            const result = await Society.create(req.body)
            res.send(result);
        } catch (err) {
            res.status(500).send(err)
        }
    ...
    delete: async (req, res) => {
        try {
            const societyId = req.params.societyId;
            const result = await Society.delete(societyId)
            res.sendStatus(200)
        } catch (err) {
            res.status(500).send(err)
        }
